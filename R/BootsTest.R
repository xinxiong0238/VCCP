...
#'  by \code{\link{VC.NBS.FindPoints}}.
#'
#' If you get the change point candidates from \code{\link{VC.NBS.FindPoints}},
#' parameters used in \code{TestPoints.Boot} are strongly recommended to be consistent
...
#' You can also combine the detection (NBS) and inference process (Stationary Bootstrap)
#' together by \code{\link{VCCP}} with \code{method="NBS"} and \code{test="B"}.
#'
#' @param v_t_point A integer vector of change point candidates, normally
#' from the first element of the list generated by \code{\link{VC.NBS.FindPoints}}.
#'
...
#' TestPoints.Boot(result, data, delta=30)
#' @seealso  \code{\link{VC.NBS.FindPoints}}, \code{\link{TestPoints.Vuong}}, \code{\link{VCCP}}
#'
#' @importFrom VineCopula RVineStructureSelect
#' @importFrom CDVine CDVineCopSelect
#' @importFrom CDVine CDVineBIC
...
#' data = cbind(1:180, random.mvn.simulate.2.changes(180,8,seed=101))
#' result = VC.NBS.FindPoints(data, 30)
#' inference = TestPoints.Boot(result[[1]], data, delta=30)
#' GetTestPlot.Boot(inference, 180)
#'
#' inference.2 = VCCP(data, method = "OBS", 30, test = "B")
#' GetTestPlot.Boot(inference.2, 180)
#' @seealso \code{\link{VCCP}}, \code{\link{TestPoints.Boot}}
