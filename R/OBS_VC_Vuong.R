      ...
      if(sum(is.element((i-delta+1):(i+delta) , cut_point0)) != 0){
        ...
        if(CDR=="R"){
          ...
          if(CDR=="C"){
            ...
            D_1 <- CDVineCopSelect(MultiInd(X, t_start, t_end), type = 2, familyset = unlist(family_set),
                                   selectioncrit = "BIC", indeptest = TRUE)
            D_2 <- CDVineCopSelect(MultiInd(X, t_start, t_point), type = 2, familyset = unlist(family_set),
                                   selectioncrit = "BIC", indeptest = TRUE)
            D_3 <- CDVineCopSelect(MultiInd(X, t_point, t_end), type = 2, familyset = unlist(family_set),
                                   selectioncrit = "BIC", indeptest = TRUE)
            BIC_de[i] <-  CDVineBIC(MultiInd(X, t_start, t_end), type = 2,
                                    family = D_1$family, par = D_1$par,par2 = D_1$par2)$BIC -
              (
                CDVineBIC(MultiInd(X, t_start, t_point), type = 2,
                          family = D_2$family, par = D_2$par,par2 = D_2$par2)$BIC +
                  CDVineBIC(MultiInd(X, t_point, t_end), type = 2,
