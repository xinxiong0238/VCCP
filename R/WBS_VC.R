...
#' Find and test change points in connection network by VCCP (WBS) model
#'
#' \code{VC_WBS} returns a dataframe containing the inference result
...
#'
#' Note that for a large M(e.g., \code{M=50}), the WBS algorithm may only
...
#'
#' @param M An integer indicating the sub-sampling size in the WBS model.
#'  If not specified(i.e.,\code{M}=NA), it selects \code{floor(9*log(T))}
