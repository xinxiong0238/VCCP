...
VC_WBS_FindPoints <- function(X_raw, delta, CDR="D", trunc_tree=NA, family_set=1, pre_white=0, ar_num=1){
  T <- length(unique(X_raw[, 1]))
  M <- floor(9*log(T))
...
#' @export
VC_WBS_Boot <- function(X_raw, delta, CDR="D", trunc_tree=NA,
                                family_set=1, p=0.3, N=100, pre_white=0, ar_num=1, sig_alpha=0.05){
  VC_WBS_FindPoints.result <- VC_WBS_FindPoints(X_raw, delta, CDR, trunc_tree,
...
#' @export
VC_WBS_Vuong <- function(X_raw, delta, CDR="D", trunc_tree=NA,
